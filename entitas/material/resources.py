...
class AdminTrainingMaterialWithTrainingIdResource:
    def on_get(self, req, resp, training_id: int):
        filters = generate_filters_resource(req=req, params_int=['id'], params_string=['name'])
        page = int(req.get_param('page', required=False, default=1))
        limit = int(req.get_param('limit', required=False, default=9))
        data, pagination = services.get_material_by_training_id_for_admin(
            page=page, limit=limit, filters=filters, training_id=int(training_id)
        )
        resouce_response_api(resp=resp, data=data, pagination=pagination)
    def on_post(self, req, resp, training_id: int):
        file = req.get_param("file")
